                    ...
                    <div class="d-flex flex-column">
                        <div class="d-flex">
                            <a href="{{ auth()->user()->path() }}">
                                <img data-src="{{ auth()->user()->pathImage() }}" class="circle img-40 mr-8 lazy" title="{{ auth()->user()->name }}">
                            </a>
                            <div class="d-flex flex-column justify-content-center">
                                <a href="{{ auth()->user()->path() }}" class="text-success text-success-hover" title="{{ auth()->user()->name }}">
                                    {{ auth()->user()->name }}
                                </a>
                        ...
                        <div class="mt-20">
                            <form action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    <input type="text" id="title" class="form-control p-0 bd-none font-size-40 font-weight-bold text-dark" name="title" placeholder="Title" autofocus autocomplete="off" value="{{ old('title') }}">
                                </div>
...
@endsection
@section('script')
    {{-- <srcipt src="assets/js/select2.min.js"></srcipt> --}}
    <script>
        // Select2
        $(document).ready(function () {
            $('#topic').select2({
                placeholder: 'Please select a topic for your post'
            });
        });
