							...
							<div class="d-flex flex-column align-items-center card-shadow">
                                <div class="d-flex justify-content-between align-items-center w-full p-3">
                                    ...
                                    @if (auth()->user()->subscribed($topic))
                                        <button id="detach" class="btn btn-action circle bg-dark-blue btn-shadow star topic" data-id="{{ $topic->id }}">
                                ...
                                <a href="{{ $topic->path() }}" class="w-full" title="{{ $topic->name }}">
                                    @if (file_exists($topic->pathImage()))
                                        <div class="h-180 m-2 bg-position-center lazy" data-src="{{ $topic->pathImage() }}"></div>
                                    @else
                                        <div class="h-180 m-2 img-error lazy" data-src="{{ $topic->pathImageError() }}"></div>
                                    @endif
                                </a>
                            </div>
						</div>
